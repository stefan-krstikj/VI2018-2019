...
class Podvizni(Problem):
    ...
    def check_valid_state(self, curr_state):
        ...
        if (covek_x == prepreka_x and covek_y == prepreka_y) or (covek_x == prepreka_x and covek_y == prepreka_y + 1):
            return False
        if (covek_x == prepreka_x + 1 and covek_y == prepreka_y) or (
                covek_x == prepreka_x + 1 and covek_y == prepreka_y + 1):
            return False
    ...
    def h(self, node):
        state = node.state
        #print(state)
        return abs(state[0][0] - self.goal[0]) + abs(state[0][1] - self.goal[0])
